...
export default function Search(props) {
  ...
  const pagCat = async (page) => {
    try {
      const { data } = await axios.post(`/api/products/category?page=${page}`, {
        search,
        category,
      });
      setProducts(data);
  ...
  const handlePageClick = (data) => {
    ...
    {
      category == "" ? pagSearch(data.selected) : pagCat(data.selected);
    }
  };
    ...
    <div>
      <Layout>
        <div className="lg:flex">
          <div className="flex flex-col w-full lg:w-64 h-screen  px-4  bg-white border-r :bg-gray-800 :border-gray-600">
