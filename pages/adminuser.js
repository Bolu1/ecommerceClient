...
function AdminUser() {
  ...
  const submitHandlder = async () => {
    ...
    try {
      const { data } = await axios.post("/api/user/search", { search });
      setProducts(data);
    } catch (err) {
      console.log(err);
    }
  ...
  return (
    <>
      <div className="lg:flex">
        <div className="flex flex-col lg:w-64 lg:h-screen  px-8 py-8 bg-slate-800 bproduct-r :bg-gray-800 :bproduct-gray-600">
                    ...
                    {order.map((order) => (
                      <>
                        <tr className="space-x-5 text-sm">
                          ...
                          <td className="p-3 md:p-5">
                            <p className=":text-coolGray-400">{order.name}</p>
                          </td>
                          <td className="p-3 md:p-5">
                            <p className=":text-coolGray-400">{order.email}</p>
                          </td>
                          <td className="p-3 md:p-5">
                            <p className=":text-coolGray-400">
                              <span>
                                {order.isAdmin ? `Admin` : "Not Admin"}
                              </span>
                            </p>
                          </td>
                          <td className="p-3 md:p-5">
                            <Link href={`/order/${order._id}`}>
                              <a className="px-8 py-3 font-semibold rounded bg-yellow-500 text-white">
                                Suspend
                              </a>
                            </Link>
                          </td>
